...
class HospitalDiagnosis(models.Model):
    ...
    def check_comments_of_mentor(self):
        """ If the intern diagnoses the patient, the mentor doctor must add
         his comment to this diagnosis."""
        for diagnosis in self:
            if diagnosis.doctor_id.mentor_id and not \
                    diagnosis.comment_of_mentor:
                raise models.ValidationError(_("Comments from mentor doctor"
                                               " are required"))

    def name_get(self):
        result = []
        for rec in self:
            date = rec.diagnosis_date
